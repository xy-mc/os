...
BaseOfLoader              equ 09900h  ; LOADER.BIN 被加载到的位置 ---- 段地址
...
LoaderFound:                     ; 找到 LOADER.BIN 后便来到这里继续
    mov    dh,4
    call   DispStr
    add    di, 01Ah              ; 0x1a = 28 这个 28 在目录项里偏移量对应的数据是起始簇
...
FindAA1InRootDir:
    ...
    mov    si, AA1FileName    ; ds:si -> "LOADER  BIN"
    mov    di, OffsetOfLoader    ; es:di -> BaseOfLoader:400h = BaseOfLoader*10h+400h
    mov    dx, 10h               ; 32(目录项大小) * 16(dx) = 512(BPB_BytsPerSec)
...
CompareAA1name:
    call   StringCmp
    cmp    ax, 1
    jz     AA1Found           ; ax == 1 -> 比对成了
    dec    dx
    cmp    dx, 0                 
    jz     GotoNextAA1DirSector ; 该扇区的所有目录项都探索完了，去探索下一个扇区
    add    di, 20h               ; 32 -> 目录项大小
    jmp    CompareAA1name

GotoNextAA1DirSector:
    inc    word [RootDirSectorNow]      ; 改变正在搜索的扇区号
    dec    word [LeftRootDirSectors]    ; ┓
    cmp    word [LeftRootDirSectors], 0 ; ┣ 判断根目录区是不是已经读完
    jz     NoLoader                     ; ┛ 如果读完表示没有找到 LOADER.BIN，就直接开摆
    jmp    FindAA1InRootDir


AA1Found:                     ; 找到 LOADER.BIN 后便来到这里继续
    mov    dh,5
    call   DispStr
    add    di, 01Ah              ; 0x1a = 28 这个 28 在目录项里偏移量对应的数据是起始簇
    mov    dx, word [es:di]      ; 起始簇号占2字节，读入到dx里
